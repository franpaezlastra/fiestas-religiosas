    ...
    >
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
        ...
        <nav className="hidden items-center gap-5 lg:flex">
          ...
          <div
            className="relative"
            onMouseEnter={() => setLibroAbierto(true)}
            onMouseLeave={() => setLibroAbierto(false)}
          >
            <button
              type="button"
              ...
              aria-haspopup="true"
              onFocus={() => setLibroAbierto(true)}
              onClick={() => setLibroAbierto((v) => !v)}
            >
              El libro
            </button>
            {libroAbierto ? (
              <div className="absolute right-0 top-full z-50 min-w-48 border border-azul-logo/20 bg-blanco py-2 text-azul-petroleo shadow-sm">
                {NAV_LIBRO.map((l) => (
                  <NavLink
                    key={l.to}
                    to={l.to}
      ...
      {menuAbierto ? (
        <nav className="border-t border-azul-logo/15 bg-blanco px-4 py-4 text-azul-petroleo lg:hidden">
          ...
          <p className="mt-4 font-display text-xs uppercase tracking-wide text-celeste-cielo">
            El libro
          </p>
